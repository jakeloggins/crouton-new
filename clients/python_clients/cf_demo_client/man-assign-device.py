...
def on_message(client, userdata, msg):
    ...
    # old method - answer a get request by sending the device JSON
    ...
    # new method - get the device JSON, store it, and send it back
    if box == "deviceInfo" and str(msg.payload) != "get" and address == "control": 
        global device
        global json
        global j
        global clientName
    ...
    if box == "inbox" and address != "deviceInfo": # and (address == "discoLights" or address == "light2"):
        ...
        # this is where RGB value fails, not sure why
...
def update_values():
    ...
    #         client.publish("/outbox/"+clientName+"/crouton-simple-text", '{"value":'+str(barDoor)+'}')
    #         barDoorDelay = counter + 5 #wait 5 seconds for next increment
    #         updateValue("crouton-simple-text","value",barDoor)
...
if __name__ == '__main__':
    ...
    # initial setup JSON is sent to /deviceInfo/ [clientName] / control
    # successful setup JSON is returned to /deviceInfo/ [clientName] / confirm

        # helper script will grab errors and send active device list to /activedevices
        # dashboard can then automatically pulldown all active devices without use of browser cookies
